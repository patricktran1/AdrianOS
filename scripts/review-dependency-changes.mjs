...
const HEAD_LOCK_PATH = "adrianos-play-starter/package-lock.json";
...
function readBaseLock() {
  try {
    return JSON.parse(
      execFileSync("git", ["show", `origin/main:${HEAD_LOCK_PATH}`], {
        encoding: "utf8",
...
if (!headLock) throw new Error(`${HEAD_LOCK_PATH} is required for dependency review.`);
...
for (const change of changes.filter((item) => item.kind !== "removed")) {
  ...
  if (
    typeof change.resolved !== "string" ||
    !change.resolved.startsWith("https://registry.npmjs.org/")
  ) {
    violations.push({
