...
async function main() {
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (!game.slug || !game.title || !game.description || !game.emoji) {
        throw new Error("Missing required metadata fields.");
      }

      if (game.slug !== entry.name) {
        throw new Error(`Slug "${game.slug}" must match folder name "${entry.name}".`);
      }

      if (!allowedSubjects.has(game.subject)) {
        throw new Error(`Unsupported subject "${game.subject}".`);
      }
      ...
      games.push({
        ...
        age: game.age ?? "Ages 6+",
        status: game.status ?? "playable",
        order: Number.isFinite(game.order) ? game.order : 999,
      });
    } catch (error) {
  ...
  const output = `// AUTO-GENERATED. DO NOT EDIT BY HAND.\nimport type { Game } from "./games";\n\nexport const games: Game[] = ${JSON.stringify(
    games.map(({ order, ...game }) => game),
    null,
    2
  )};\n`;
