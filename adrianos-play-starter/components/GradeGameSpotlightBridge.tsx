  ...
  return (
    <section style={card} aria-label="Featured grade adventure">
      ...
      <div style={copy}>
        ...
        <div style={mechanics}>
          {featured.mechanics.map((mechanic) => <span key={mechanic}>{mechanic}</span>)}
        </div>
      </div>
...
const card: React.CSSProperties = { width: "min(1040px,calc(100% - 36px))", margin: "16px auto", padding: "clamp(20px,4vw,32px)", display: "grid", gridTemplateColumns: "auto minmax(0,1fr) minmax(190px,auto)", gap: 22, alignItems: "center", borderRadius: 30, background: "linear-gradient(135deg,rgba(127,220,255,.13),rgba(217,255,91,.08),#181d28 72%)", border: "1px solid rgba(127,220,255,.3)", color: "#fff", overflow: "hidden" };
const visual: React.CSSProperties = { fontSize: "clamp(4rem,10vw,7rem)", filter: "drop-shadow(0 14px 25px rgba(0,0,0,.3))" };
...
Object.assign(mechanics, { color: "#aab1bf", fontSize: 11, fontWeight: 850 });

if (typeof window !== "undefined" && window.matchMedia?.("(max-width: 720px)").matches) {
  Object.assign(card, { gridTemplateColumns: "1fr", textAlign: "center" });
  Object.assign(visual, { margin: "0 auto" });
  Object.assign(mechanics, { justifyContent: "center" });
}
