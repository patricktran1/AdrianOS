...
const COLLECTIONS: Record<ElementaryGrade, { title: string; intro: string; prizes: Array<{ emoji: string; name: string }> }> = {
  0: {
    title: "Critter Parade",
    intro: "Every finished game invites a new parade friend.",
    prizes: [["🐛", "Wiggle Bug"], ["🐸", "Hop Frog"], ["🐞", "Dot Beetle"], ["🦋", "Sky Flutter"], ["🐌", "Turbo Snail"], ["🐝", "Buzz Buddy"], ["🐢", "Tiny Turtle"], ["🦀", "Clap Crab"], ["🐙", "Wavy Octopus"], ["🦖", "Baby Dino"], ["🦄", "Cloud Unicorn"], ["🐉", "Mini Dragon"]].map(([emoji, name]) => ({ emoji, name })),
  },
  1: {
    title: "Rainbow Rocket Crew",
    intro: "Finish adventures to fill the rocket with a colorful crew.",
    prizes: [["🌈", "Rainbow Fuel"], ["🚀", "Pocket Rocket"], ["⭐", "Wish Star"], ["🌙", "Moon Pal"], ["🪐", "Ring Planet"], ["☄️", "Comet Dash"], ["👽", "Friendly Alien"], ["🛸", "Mini Saucer"], ["🌞", "Sunny Captain"], ["🔭", "Star Finder"], ["🛰️", "Space Scout"], ["🌌", "Galaxy Gem"]].map(([emoji, name]) => ({ emoji, name })),
  },
  2: {
    title: "Robot Upgrade Deck",
    intro: "Each clear installs a new robot upgrade.",
    prizes: [["🤖", "Helper Bot"], ["⚙️", "Mega Gear"], ["🔋", "Power Cell"], ["🦾", "Strong Arm"], ["🛞", "Turbo Wheels"], ["📡", "Signal Dish"], ["💡", "Idea Bulb"], ["🧲", "Magnet Grip"], ["🛡️", "Shield Plate"], ["🔧", "Fix-It Tool"], ["🎛️", "Control Board"], ["🏆", "Golden Core"]].map(([emoji, name]) => ({ emoji, name })),
  },
  3: {
    title: "Dragon Treasure Hoard",
    intro: "Every completed quest adds treasure to the hoard.",
    prizes: [["🥚", "Dragon Egg"], ["🔥", "Fire Spark"], ["🗝️", "Castle Key"], ["💎", "Blue Gem"], ["👑", "Tiny Crown"], ["🧭", "Quest Compass"], ["🗡️", "Hero Sword"], ["🛡️", "Knight Shield"], ["🏰", "Pocket Castle"], ["📜", "Secret Map"], ["🐲", "Dragon Friend"], ["🏆", "Royal Trophy"]].map(([emoji, name]) => ({ emoji, name })),
  },
  4: {
    title: "Space Station Specimens",
    intro: "Successful missions recover rare objects from deep space.",
    prizes: [["🧪", "Meteor Sample"], ["🛰️", "Scout Satellite"], ["🪐", "Gas Giant"], ["🌑", "Dark Moon"], ["☄️", "Ice Comet"], ["👾", "Pixel Alien"], ["🔭", "Deep Lens"], ["🧬", "Alien Helix"], ["🚀", "Research Shuttle"], ["🌟", "Supernova"], ["🕳️", "Black Hole"], ["🌌", "Sigma Galaxy"]].map(([emoji, name]) => ({ emoji, name })),
  },
  5: {
    title: "Temple Relic Gallery",
    intro: "Each victory restores one relic to the gallery.",
    prizes: [["🗿", "Stone Guardian"], ["🏺", "Ancient Vase"], ["🪬", "Oracle Eye"], ["🧿", "Blue Talisman"], ["📜", "Rune Scroll"], ["🗝️", "Vault Key"], ["💎", "Sun Gem"], ["🧭", "Lost Compass"], ["⚱️", "Golden Urn"], ["🪶", "Scribe Feather"], ["☀️", "Sun Dial"], ["👑", "Temple Crown"]].map(([emoji, name]) => ({ emoji, name })),
  },
  6: {
    title: "Cyber City Artifact Grid",
    intro: "Completed runs decrypt one artifact at a time.",
    prizes: [["💾", "Data Disk"], ["🧩", "Code Fragment"], ["🔐", "Cipher Lock"], ["🛡️", "Firewall Badge"], ["🧠", "Neural Chip"], ["📡", "Quantum Relay"], ["🎮", "Arcade Core"], ["⚡", "Power Node"], ["🤖", "City Drone"], ["🕶️", "Holo Visor"], ["🌐", "Network Globe"], ["💠", "Master Crystal"]].map(([emoji, name]) => ({ emoji, name })),
  },
};

function totalCompletions(games: ReturnType<typeof useAdrianProgress>["progress"]["games"]): number {
  return Object.values(games).reduce((sum, game) => sum + game.completions, 0);
}
...
export default function PrizeVault() {
  ...
  const clears = totalCompletions(progress.games);
  ...
  const message = useMemo(() => {
    ...
    if (unlocked < collection.prizes.length) return `${collection.prizes.length - unlocked} prize${collection.prizes.length - unlocked === 1 ? "" : "s"} still hidden.`;
    return prestige > 0 ? `Collection complete · ${prestige} champion star${prestige === 1 ? "" : "s"} earned.` : "Collection complete. Replays now earn champion stars.";
  ...
  return (
    <section aria-label="Prize Vault" style={{ ...shell, ...(celebrate ? celebration : {}) }}>
      <div style={headingRow}>
        <div>
          <span style={eyebrow}>PRIZE VAULT · {elementaryGradeLabel(grade)}</span>
          <h2 style={title}>{collection.title}</h2>
          <p style={copy}>{collection.intro}</p>
        </div>
      ...
      {celebrate && unlocked > 0 && (
        <div role="status" style={newPrizeBanner}>
          ...
          <span><strong>New prize unlocked!</strong><small>{collection.prizes[unlocked - 1].name} joined your collection.</small></span>
      ...
      <div style={grid}>
        {collection.prizes.map((prize, index) => {
          ...
          return (
            <div key={prize.name} style={{ ...prizeCard, ...(open ? openCard : lockedCard) }} aria-label={open ? `${prize.name} unlocked` : "Locked prize"}>
              <span style={prizeEmoji} aria-hidden="true">{open ? prize.emoji : "?"}</span>
              <strong>{open ? prize.name : "Mystery prize"}</strong>
              <small>{open ? `Clear ${index + 1}` : `Finish ${index + 1 - clears} more`}</small>
            </div>
          );
        })}
      </div>
      ...
      <div style={footerRow}>
        ...
        {prestige > 0 && <strong>⭐ {prestige} champion star{prestige === 1 ? "" : "s"}</strong>}
      </div>
    </section>
  );
}
...
const prizeCard: React.CSSProperties = { minHeight: 132, display: "grid", placeItems: "center", alignContent: "center", gap: 5, padding: 12, borderRadius: 20, textAlign: "center", transition: "transform .2s ease" };
