      ...
      {celebrate && unlocked > 0 && (
        <div role="status" style={newPrizeBanner}>
          ...
          <span>
            ...
            <small>{collection.prizes[unlocked - 1].name} joined your collection.</small>
          </span>
      ...
      <div style={grid}>
        {collection.prizes.map((prize, index) => {
          ...
          return (
            <div
              key={prize.name}
              style={{ ...prizeCard, ...(open ? openCard : lockedCard) }}
              aria-label={open ? `${prize.name} unlocked` : "Locked prize"}
            >
              <span style={prizeEmoji}>{open ? prize.emoji : "?"}</span>
              <strong>{open ? prize.name : "Mystery prize"}</strong>
              <small>{open ? `Clear ${index + 1}` : `Finish ${Math.max(1, index + 1 - clears)} more`}</small>
            </div>
          );
        })}
      </div>
