...
export default function WordForgeStudio() {
  ...
  function check() {
    ...
    recordLearningAttempt({ gameSlug: GAME_SLUG, subject: "Reading", skillId: `spelling-grade-${grade}`, skillLabel: "Word construction and morphology", prompt: `${card.clue} Build the word.`, correctAnswer: card.word, correct, givenAnswer: built.toLowerCase(), wrongAttempts: misses, data: { grade: grade ?? 0, standardCode: card.standard, chunk: card.chunk, supportUsed: misses > 0 } }, activeProfile.id);
    if (!correct) {
  ...
  if (!started) return <GameFrame title={pack.title}><main style={{ ...page, background: `radial-gradient(circle at top,${pack.accent}30,#11151d 55%)` }}><section style={hero}><div style={big}>{pack.emoji}</div><span style={{ ...eyebrow, color: pack.accent }}>WORD BUILDING</span><h1 style={title}>{pack.title}</h1><p style={lead}>{pack.intro}</p><div style={stats}><strong>🔤 5 words</strong><strong>🔥 combo rewards</strong><strong>🔊 hear every word</strong></div><button onClick={() => setStarted(true)} style={{ ...primary, background: pack.accent }}>Start forging →</button></section></main></GameFrame>;
